...
- [C++][basic] 空类占多大内存
- [C++][basic] 如何调用父类的虚函数？
- [C++][Advanced] 继承（内存长什么样，有/无虚函数时又长什么样）
...
- [C++][Advanced] 多态（静态/动态多态，虚函数，虚表，原理）
- [C++][Advanced] 虚函数指针存储在哪里
...
- [C++][Advanced] 构造函数能声明为虚函数吗？析构函数呢
- [C++][Advanced] 父类构造函数调用父类 virtual 时，调用的是哪个函数（父的还是子类重写的）
...
- [C++][Advanced] 内存对齐的规则
- [C++][Advanced] 类大小计算（内存对齐）
- [C++][Advanced] 栈什么情况会爆
- [C++][Advanced] 内存碎片，怎么设计一个内存管理系统避免内存碎片
- [C++][Advanced] delete 是怎么知道要 delete 多少字节
- [C++][Advanced] new int() 和 new int 的区别
- [C++][Advanced] placement new
- [C++][Advanced] 如何检查内存泄漏
- [C++][Advanced] 如何通过重载 new 和 delete 检测内存泄漏
- [C++][Advanced] C++ 的内存模型
- [C++][Advanced] 类的成员变量存储位置：类定义本身在代码区，实例化对象的成员在堆/栈上取决于对象分配方式
...
- [C++][basic] 介绍平时使用的容器
- [C++][basic] STL 你常用哪些容器？
- [C++][Advanced] vector 是如何动态扩容的？
- [C++][Advanced] vector 扩容机制
- [C++][Advanced] vector 和 list 的区别（局部性、性能、结构）
- [C++][Advanced] vector 中去重（erase 会有什么问题、迭代器）
- [C++][Advanced] 如果自己实现一个 vector，你会怎么考虑
- [C++][Advanced] map 的底层是什么，复杂度（红黑树）
- [C++][Advanced] unordered_map / map（底层结构、哈希冲突怎么解决）
- [C++][Advanced] STL 的排序算法用的是什么？
...
- [C++][Advanced] map 迭代遍历结果是有序的（红黑树中序遍历）

### 模板
- [C++][basic] 模板有什么好处
- [C++][Advanced] 模板的特化与偏特化
- [C++][Advanced] 模板能不能声明定义分开写
- [C++][Advanced] 模板什么时候知道 T 的具体类型
...
- [C++][Advanced] 构造函数能不能为 virtual
- [C++][Advanced] 析构函数为什么要是虚函数
- [C++][Advanced] 如果不通过 delete，离开作用域时自动销毁，会调用派生类的析构函数吗
- [C++][Advanced] 析构时先析构子还是父
...
- [C++][Advanced] C++ 四种 cast
- [C++][Advanced] static_cast 和 dynamic_cast 转换失败的区别（编译报错 / 指针返 null / 引用抛 bad_cast）
- [C++][Advanced] 两个无关联的 class 如何互相转换（需提供构造函数或类型转换运算符）
- [C++][basic] 引用传参传的是什么
- [C++][Advanced] i++ 和 ++i 哪个是左值哪个是右值
- [C++][Advanced] 左值右值、移动语义
- [C++][Advanced] 移动构造后需要将原对象的指针置空
...
- [C++][Advanced] 死锁，线程同步
- [C++][Advanced] 进程崩溃和线程崩溃有什么区别
- [C++][Advanced] 进程访问的内存地址是实际的真实的内存地址吗
- [C++][Advanced] 有没有使用过多线程编程
...
- [C#][basic] 装箱 / 拆箱
- [C#][Advanced] 结构体是否可以继承 / 实现接口？
- [C#][Advanced] 把一个实现接口的结构体实例赋值给接口变量可以吗？如果可以会产生什么问题？
- [C#][Advanced] 一个值类型可以存储在堆上吗

### 委托与事件
关联: DesignPatterns
- [C#][basic] 委托和事件的关系，如何用 Action 编写一个简单的事件订阅系统
- [C#][basic] 你对 C# 的委托理解
- [C#][Advanced] 委托和事件的区别是什么？在实际项目中如何选择使用
- [C#][Advanced] 委托怎么删除某个方法
...
- [C#][Advanced] 假如有一个算法需要多次生成数组（每个 ~20 元素），如何避免频繁 GC
- [C#][Advanced] GC 触发与实践中怎么避免？
- [C#][Advanced] C# 的 GC 是什么，平时有什么编码习惯可以减少 GC
- [C#][Advanced] C# 非托管内存
- [C#][Advanced] 如果对象 A 和对象 B 相互依赖并且确定要被 GC 收集，会发生什么
...
- [C#][Advanced] 泛型委托使用的方法
- [C#][Advanced] C# 中 List 的底层实现，增删改的时间复杂度，不保证顺序时怎么优化删除
- [C#][Advanced] C# List 和 C++ vector 扩容区别：C# 不立即释放原数组（依赖 GC），C++ 立即 delete
- [C#][Advanced] C# Dictionary 对标 C++ unordered_map（哈希表 O(1)），C++ map 为红黑树 O(logn)
...
- [Lua][Advanced] Lua 闭包的实现机制（upvalue，open 状态引用栈变量、close 状态拷贝到自身结构体）

### 元表与元方法
- [Lua][basic] 元表的作用和常见元方法（__index、__add、__tostring 等）
- [Lua][Advanced] Lua 如何用 table + 元表实现面向对象（__index 实现继承，重写元方法实现多态）
...
- [Unity][Advanced] 为什么 Awake 访问对象会是 null
- [Unity][Advanced] 如何解决初始化顺序问题
...
- [Unity][Advanced] Unity 的协程和线程、进程有啥区别？
- [Unity][Advanced] 100 个协程被挂起，现在有 10 个协程满足条件，应该先执行哪一个
...
- [Unity][Advanced] UGUI 事件响应流程：点击屏幕 → 射线检测（Raycast） → 命中 UI 元素 → 触发 IPointerClickHandler 等回调
- [Unity][Advanced] Mask 和 RectMask2D 的区别
- [Unity][Advanced] UGUI 中的 Mask 遮罩是怎么实现的
- [Unity][Advanced] UGUI 重绘
- [Unity][Advanced] Text 和 TextMeshPro 的区别是什么？TextMeshPro 是如何渲染一个汉字的？
- [Unity][Advanced] 动态循环列表是怎么实现的？为什么要用？有什么优点？
...
- [Unity][Advanced] UGUI 优化的方法
- [Unity][Advanced] UI 合批条件：不能被打断、不能穿插、要一个图集
- [Unity][Advanced] UI 元素重叠（图片+文字）会打断合批，增加 DrawCall
- [Unity][Advanced] UI Rebuild 优化：减少 Canvas 网格重新构建，避免 CPU 频繁计算
- [Unity][Advanced] 一个复杂的 UI 场景里，两个静态 UI 之间有很多动态 UI 且有遮挡，按动静分离是不是要打很多子 Canvas
...
- [Unity][Advanced] 在 2D AI 物理移动时，为什么通常将逻辑写在 FixedUpdate 而不是 Update 中
- [Unity][Advanced] FixedUpdate 是如何执行的？如果 FixedUpdate 的间隔特别短，如何保证严格根据物理时间执行
- [Unity][Advanced] 高速移动下的物体，如何避免穿模
- [Unity][Advanced] Unity 物理系统的原理
...
- [Unity][Advanced] AB 包（打包注意什么、加载卸载要注意什么、引用计数）
- [Unity][Advanced] Unity 预制体记录资源引用的方式
- [Unity][Advanced] 打包时如何获取到资源在哪个 AB 上面
- [Unity][Advanced] 读取 AB 时，如何知道资源属于哪个 AB
- [Unity][Advanced] AB 包的颗粒度是怎么拆分的
- [Unity][Advanced] 同步加载和异步加载都会产生内存方面的问题，怎么优化？异步产生的内存用什么方式释放
- [Unity][Advanced] Unity 打包的流程和注意事项（AssetBundle 构建、资源冗余如何处理）
- [Unity][Advanced] Unity 什么操作会产生大量 GC
...
- [Unity][Advanced] Unity 动画系统的原理
- [Unity][Advanced] Animation 和 Animator 的区别，在只有几个简单动画切换情况下哪种性能更好，为什么
- [Unity][Advanced] Unity 的 Animator 中的动画会全部加载到内存中吗？怎么优化
- [Unity][Advanced] Playable 相关

### 寻路与 NavMesh
关联: Algorithms · GameDesign
- [Unity][basic] NavMesh 是离线状态下烘焙好寻路网格的，遇到移动的障碍物如何让 NPC 不撞墙
- [Unity][Advanced] 假设有两层楼，二楼有一个洞，如何让 NPC 从洞掉到一楼还能继续正常寻路
- [Unity][Advanced] Unity Navigation 寻路的原理

### ScriptableObject
关联: C#
- [Unity][basic] 什么是 ScriptableObject？相比于在 Prefab 的 MonoBehaviour 面板上直接配置数值有什么优势
- [Unity][Advanced] ScriptableObject 的优劣势，什么情况下用，什么情况下不用
...
### 摄像机
关联: Graphics
- [Unity][Advanced] 在 3D 角色到墙角后，怎么调试可以让摄像机不被挤到外面或穿模
- [Unity][Advanced] 摄像机避开障碍物的实现
...
- [Graphics][Advanced] 欧拉角、四元数、旋转矩阵的区别和联系

### 深度测试
- [Graphics][basic] 什么是深度测试？在渲染管线的哪个阶段进行
- [Graphics][basic] 深度测试是在什么阶段实现的
- [Graphics][Advanced] Early-Z 什么时候会失效

### 透明混合
- [Graphics][basic] 透明混合
- [Graphics][Advanced] 如果要实现半透明的话 Shader 代码怎么写

### 前向 / 延迟渲染
- [Graphics][basic] 前向渲染和延迟渲染的区别、各自优缺点、适用场景
- [Graphics][Advanced] 延迟渲染的原理、优缺点
...
- [Graphics][Advanced] 判断点在三角形内（叉积、重心坐标、面积法）
- [Graphics][Advanced] 判断点是否在三角形内部
- [Graphics][Advanced] 射线检测的原理，如何自己实现

### 法线贴图
- [Graphics][basic] 什么是法线贴图？它是如何工作的？为什么通常看起来是蓝色的
- [Graphics][Advanced] 法线贴图（法线怎么存的、为什么多为蓝色）
...
- [Algorithms][Advanced] 除了开放寻址和链地址还有吗（再哈希）
- [Algorithms][Advanced] 哈希表最差情况复杂度，哈希冲突怎么解决
...
- [Algorithms][Advanced] AStar（流程、是否最优）
- [Algorithms][Advanced] A* 算法的优势
- [Algorithms][Advanced] A* 原理及与流场寻路对比

### 链表
- [Algorithms][basic] 链表与数组的区别
- [Algorithms][Advanced] 判断链表是否有环、找到环入口（怎么证明）
- [Algorithms][Advanced] 不使用快慢指针呢？链表中每个节点值不同（存哈希表）

### 红黑树与平衡树
关联: C++
- [Algorithms][Advanced] 平衡二叉树的概念、左右子树深度差值是多少
- [Algorithms][basic] AVL 树
- [Algorithms][Advanced] AVL 树如何保证平衡性质
- [Algorithms][Advanced] 手撕平衡二叉树的判断，要求自己构建树结构
- [Algorithms][Advanced] 红黑树的原理

### Top-K
- [Algorithms][basic] Top-K 问题（找前 10 最小），复杂度
- [Algorithms][Advanced] 有 1 万个玩家的排名数据（分数），想取前 100 名，有什么快速方法

### 空间划分
关联: Performance
- [Algorithms][Advanced] 有一堆 2D 坐标，快速找有哪些落在半径为 r 的圆范围内
...
- [Networking][Advanced] 如何实现一个轻量的可靠 UDP
- [Networking][Advanced] UDP TCP 具体的应用，哪些模块适合用哪一种，具体游戏开发中如何保证可靠性
...
- [Networking][Advanced] TCP 的四次挥手能不能改成三次，为什么
- [Networking][Advanced] 既然 UDP 不可靠，有没有方式让它变得可靠一点
- [Networking][Advanced] 超时重传失败后会怎样
...
- [DesignPatterns][Advanced] 观察者模式什么时候使用、怎么使用、方法封装使用哪些 API
- [DesignPatterns][Advanced] 观察者和单例，优缺点，在项目中的应用

### 单例模式
- [DesignPatterns][Advanced] 观察者和单例，优缺点，在项目中的应用
- [DesignPatterns][Advanced] 单例模式有什么缺点

### 享元模式
关联: Unity
- [DesignPatterns][Advanced] 享元模式怎么实现
- [DesignPatterns][Advanced] 享元模式的其他数据存在哪里

### 对象池
关联: C# · Performance
- [DesignPatterns][Advanced] 对象池要提供哪些接口

### MVC / MVVM
关联: C# · Unity
- [DesignPatterns][basic] MVC
- [DesignPatterns][Advanced] MVC 与 MVVM 区别
- [DesignPatterns][Advanced] MVVM 怎么做到 UI 绑定的

### 工厂模式
- [DesignPatterns][Advanced] 工厂模式的使用场景和实现方式

### 事件系统
关联: C#
- [DesignPatterns][basic] 简单说一下你们的事件管理是怎么做的
- [DesignPatterns][Advanced] 数据驱动
...
- [GameDesign][Advanced] FSM 和行为树的差异，实现简单的巡逻保安 AI 会选择什么模式
- [GameDesign][Advanced] 多层级 FSM

### 攻击判定
关联: Networking
- [GameDesign][basic] 获取攻击目标的方法是怎样的
- [GameDesign][Advanced] 两种近战攻击判定机制：碰撞器检测 vs 帧动画关键帧范围检测，优缺点
- [GameDesign][Advanced] 如果攻击判定依赖帧实现，网络波动导致攻击丢失怎么办
- [GameDesign][Advanced] 如果实现类似《空洞骑士》里面怪物遇到墙壁或走到平台边缘掉头，实现方法是什么

### 背包系统
- [GameDesign][Advanced] 背包系统的实现
- [GameDesign][Advanced] 背包中使用了一个物品的逻辑

### Buff 系统
- [GameDesign][Advanced] Buff 系统怎么设计

### 防作弊
关联: Networking
- [GameDesign][Advanced] 如果有人用外挂篡改客户端数据，怎么保证外挂无法得逞

### 场景与内存
关联: Unity
- [GameDesign][Advanced] 一个跑酷游戏不断生成场景，怎么设置让内存不崩溃
...
- [Performance][Advanced] 一个游戏场景中有一百个人，要怎么渲染才能不卡顿
...
- [Performance][Advanced] 我们要上线一个手游，在低端机上运行会发热，怎么解决

### GC 优化
关联: C# · DesignPatterns
- [Performance][Advanced] Lua GC
